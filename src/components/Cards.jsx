...
const Cards = ({ item }) => {
  ...
  const handleAddToCart = item => {
    ...
    if(user && user.email){
        ...
        axios.post('https://demo-foodie-server.vercel.app/carts', cartItem)
        .then((response) => {
  ...
  return (
    <div to={`/menu/${item._id}`} className="card shadow-xl relative mr-5 md:my-5">
      ...
      <Link to={`/menu/${item._id}`}>
        <figure>
          <img src={item.image} alt="Shoes" className="hover:scale-105 transition-all duration-300 md:h-44" />
        </figure>
      </Link>
